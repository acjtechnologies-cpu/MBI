...
console.log('Bloc trouve, longueur:', oldBlock.length);
...
const newBlock = `<div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <div className="text-center" style={{borderTop:'1px solid #374151',paddingTop:6}}>
            ...
            <div style={{fontSize:9,color:'#6b7280'}}>{Math.round(((model.masse_ref_8ms||3.474)-3.474)*1000)>0?'+':''}{Math.round(((model.masse_ref_8ms||3.474)-3.474)*1000)}g vs Pike</div>
          </div>
        </div>`;
