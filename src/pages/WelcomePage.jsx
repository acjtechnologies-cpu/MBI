...
const GLIDERS_LOCAL = [
  {
    ...
    borderColor: 'rgba(220,110,10,0.9)',
    bg: 'rgba(20,10,0,0.75)',
    badgeBg: 'rgba(220,110,10,0.35)',
    badgeColor: '#ffaa50',
  },
  {
    id: 'mamba-s',
    ...
    badge: 'Poly4 · IRP · Matrix',
    borderColor: 'rgba(80,120,220,0.9)',
    bg: 'rgba(10,18,50,0.75)',
    badgeBg: 'rgba(80,120,220,0.35)',
    badgeColor: '#aac0ff',
  },
]

// ── GliderBrowser (modal GitHub) ──────────────────────────────────────────────
function GliderBrowser({ onClose, onImport }) {
  ...
  useEffect(() => {
    ...
    fetch(INDEX_URL)
      ...
      .then(data => {
        if (cancelled) return
        if (!Array.isArray(data?.planeurs)) throw new Error('Format invalide')
        setList(data.planeurs)
        setLoading(false)
      })
      .catch(e => {
  ...
  const handleImport = async (planeur) => {
    ...
    try {
      ...
      if (!data?.id) throw new Error('Format modèle invalide')
      onImport(data)
      onClose()
    } catch (e) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button
          onClick={onClose}
          style={{color:'rgba(255,255,255,0.6)',fontSize:28,background:'none',border:'none',
      ...
      <div style={{flex:1,overflowY:'auto',padding:'14px 16px'}}>
        ...
        {!loading && !error && list.map(p => (
          <div key={p.id} style={{
            ...
            <div style={{fontSize:26,flexShrink:0}}>{p.drapeau}</div>
            <div style={{flex:1}}>
              <div style={{color:'#fff',fontSize:14,fontWeight:500}}>{p.nom}</div>
              <div style={{color:'rgba(255,255,255,0.4)',fontSize:11,marginTop:2}}>
                {p.constructeur} · {p.masseVide}g
              </div>
            </div>
            <button
...
export default function WelcomePage({ onSelect }) {
  ...
  return (
    <div style={{
      ...
      {/* Nuages déco */}
      <div style={{position:'absolute',top:44,left:14,width:80,height:26,background:'rgba(255,255,255,0.75)',borderRadius:16,pointerEvents:'none'}} />
      <div style={{position:'absolute',top:32,left:28,width:44,height:30,background:'rgba(255,255,255,0.75)',borderRadius:'50%',pointerEvents:'none'}} />
      <div style={{position:'absolute',top:68,right:20,width:58,height:18,background:'rgba(255,255,255,0.55)',borderRadius:16,pointerEvents:'none'}} />
      <div style={{position:'absolute',top:58,right:32,width:32,height:22,background:'rgba(255,255,255,0.55)',borderRadius:'50%',pointerEvents:'none'}} />
      ...
      <div style={{position:'relative',zIndex:1,padding:'0 14px 24px'}}>
        <div style={{color:'rgba(255,255,255,0.55)',fontSize:10,letterSpacing:2.5,marginBottom:10,paddingLeft:2}}>
          MON PLANEUR
        </div>
        ...
        {GLIDERS_LOCAL.map(g => (
          <button
            key={g.id}
            onClick={() => handleLocalSelect(g.id)}
            style={{
          ...
          >
            <img
              src={IMG_MAP[g.id]} alt=""
              style={{position:'absolute',right:-20,top:'50%',transform:'translateY(-50%) rotate(-5deg)',width:'65%',opacity:0.9,pointerEvents:'none'}}
            />
            ...
            <div style={{position:'relative',zIndex:2,padding:'16px'}}>
              ...
              <div style={{fontSize:10,padding:'3px 8px',borderRadius:10,marginTop:6,display:'inline-block',background:g.badgeBg,color:g.badgeColor}}>
                {g.badge}
              </div>
            </div>
        ...
        <div style={{color:'rgba(255,255,255,0.2)',fontSize:10,textAlign:'center',marginTop:8}}>
          ACJ Technologies · ESP32 v12
        </div>
      </div>
