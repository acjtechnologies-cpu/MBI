...
export default function WelcomePage() {
  const setGlider = useAppStore(s => s.setGlider)
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          fontFamily: 'monospace', fontSize: 54, fontWeight: 500,
          color: '#fff', letterSpacing: 10, lineHeight: 1,
          textShadow: '0 2px 24px rgba(0,0,0,0.25)',
        }}>F3F</div>
        <div style={{
          fontFamily: 'monospace', fontSize: 32, fontWeight: 500,
          color: '#fff', letterSpacing: 6, marginTop: -6,
          textShadow: '0 2px 24px rgba(0,0,0,0.25)',
        }}>PIT</div>
        <div style={{fontSize: 11, color: 'rgba(255,255,255,0.82)', letterSpacing: 3, marginTop: 5}}>
          BALLAST · MÉTÉO · CHRONO · IQA
        </div>
        <div style={{
      ...
      <div style={{position:'relative',zIndex:1,padding:'0 14px 28px'}}>
        ...
        {/* Pike Precision 2 */}
        <div
          onClick={() => setGlider('pike2')}
          style={{
            borderRadius: 18, padding: 16, marginBottom: 10,
            display: 'flex', alignItems: 'center', gap: 12,
            background: 'rgba(20,10,0,0.92)',
            border: '1.5px solid rgba(220,110,10,0.75)',
            position: 'relative', overflow: 'hidden', minHeight: 82,
            cursor: 'pointer',
          }}
        >
          <img src={pikeImg} alt="" style={{
            position: 'absolute', right: -15, top: '50%',
            transform: 'translateY(-50%) rotate(-6deg)',
            width: 175, mixBlendMode: 'multiply', opacity: 0.9,
            pointerEvents: 'none',
          }} />
          <div style={{position:'relative',zIndex:2,flex:1}}>
            <div style={{fontSize:15,fontWeight:500,color:'#fff'}}>Pike Precision 2</div>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.5)',marginTop:2}}>2350g · CG 96mm · 3 soutes</div>
            <div style={{
              fontSize:10,padding:'3px 8px',borderRadius:10,marginTop:5,display:'inline-block',
              background:'rgba(220,110,10,0.3)',color:'#ffaa50',
            }}>Poly4 · IRP · Matrix</div>
          </div>
        ...
        {/* Mamba S */}
        <div
          onClick={() => setGlider('mamba')}
          style={{
            borderRadius: 18, padding: 16, marginBottom: 10,
            display: 'flex', alignItems: 'center', gap: 12,
            background: 'rgba(10,18,50,0.92)',
            border: '1.5px solid rgba(80,120,220,0.6)',
            position: 'relative', overflow: 'hidden', minHeight: 82,
            cursor: 'pointer',
          }}
        >
          <img src={mambaImg} alt="" style={{
            position: 'absolute', right: -10, top: '50%',
            transform: 'translateY(-48%) rotate(-5deg)',
            width: 180, mixBlendMode: 'multiply', opacity: 0.8,
            pointerEvents: 'none',
          }} />
          <div style={{position:'relative',zIndex:2,flex:1}}>
            <div style={{fontSize:15,fontWeight:500,color:'#fff'}}>Mamba S</div>
            <div style={{fontSize:11,color:'rgba(255,255,255,0.5)',marginTop:2}}>2550g · CG 102mm · 2 soutes</div>
            <div style={{
              fontSize:10,padding:'3px 8px',borderRadius:10,marginTop:5,display:'inline-block',
              background:'rgba(80,120,220,0.28)',color:'#aac0ff',
            }}>Poly4 · IRP · Matrix</div>
          </div>
          <div style={{position:'relative',zIndex:2,color:'rgba(255,255,255,0.3)',fontSize:20,flexShrink:0}}>›</div>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'8px 0 0',cursor:'pointer'}}>
          <div style={{
            ...
            color:'rgba(255,255,255,0.38)',fontSize:15,lineHeight:1,
          }}>+</div>
          <span style={{color:'rgba(255,255,255,0.38)',fontSize:12,letterSpacing:1}}>Ajouter un modèle</span>
        </div>

        <div style={{color:'rgba(255,255,255,0.2)',fontSize:10,textAlign:'center',marginTop:12}}>
