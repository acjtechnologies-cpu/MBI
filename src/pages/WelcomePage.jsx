...
function GliderBrowser({ onClose, onImport }) {
  ...
  useEffect(() => {
    ...
    fetch(INDEX_URL)
      ...
      .then(data => {
        if (cancelled) return
        if (!data || !Array.isArray(data.planeurs)) throw new Error('Format index invalide')
        setList(data.planeurs)
        setLoading(false)
      })
      .catch(() => {
        if (!cancelled) { setError('Impossible de charger la liste'); setLoading(false) }
      })
  ...
  const handleImport = async (planeur) => {
    ...
    try {
      ...
      if (!data || !data.id) throw new Error('Format modèle invalide')
      onImport(data)
      onClose()
    } catch(e) {
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:100,background:'rgba(0,0,0,0.88)',display:'flex',flexDirection:'column',fontFamily:'sans-serif'}}>
      <div style={{padding:'18px 16px 14px',borderBottom:'1px solid rgba(255,255,255,0.1)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <button onClick={onClose} style={{color:'rgba(255,255,255,0.5)',fontSize:28,background:'none',border:'none',cursor:'pointer',padding:'4px 10px',lineHeight:1,touchAction:'manipulation',WebkitTapHighlightColor:'transparent'}}>×</button>
      </div>
        ...
        {!loading && !error && list.map(p => (
          <div key={p.id} style={{background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.12)',borderRadius:14,padding:'14px 16px',marginBottom:10,display:'flex',alignItems:'center',gap:12}}>
            ...
            <button onClick={() => handleImport(p)} disabled={importing===p.id}
              style={{background:'rgba(80,160,255,0.25)',border:'1px solid rgba(80,160,255,0.5)',borderRadius:10,padding:'8px 14px',color:'#7ac0ff',fontSize:12,fontWeight:600,cursor:'pointer',touchAction:'manipulation',WebkitTapHighlightColor:'transparent'}}>
...
export default function WelcomePage({ onSelect }) {
  ...
  const handleLocalSelect = (gliderId) => {
      ...
      <div style={{position:'fixed',inset:0,background:'linear-gradient(170deg,#4a9fd4 0%,#87CEEB 25%,#9dd4e8 50%,#6aaa60 78%,#4a8a40 100%)',display:'flex',flexDirection:'column',fontFamily:'sans-serif',userSelect:'none'}}>
        ...
        <div style={{position:'relative',zIndex:1,padding:'0 14px 28px'}}>
          ...
          <button onClick={() => setShowBrowser(true)}
            style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'10px 0',cursor:'pointer',background:'none',border:'none',width:'100%',touchAction:'manipulation',WebkitTapHighlightColor:'transparent'}}>
