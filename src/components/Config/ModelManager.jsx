...
export default function ModelManager() {
  ...
  const handleCreateSoute = () => {
    ...
    const newSoute = {
      ...
      nom: 'Nouvelle Soute',
      couleur: '#6b7280',
      distanceBA: activeModel.cgVide || 100,
      capacite: 6,
      materiaux: [
    ...
    addSoute(activeModel.id, newSoute)
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-gray-950">
      {/* Header - Sélection modèle */}
      <div className="bg-gray-900 border-b border-gray-800 p-4">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-blue-400">📋 MODÈLES</h2>
          <button
            onClick={handleCreateModel}
        ...
        <select
          ...
          className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2 text-white font-semibold"
        >
          {modelList.map(model => (
            <option key={model.id} value={model.id}>
              {model.drapeau} {model.nom}
            </option>
          ))}
        </select>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <ModelInfoCard
          ...
          onSave={(updates) => {
            updateModel(activeModel.id, updates)
        ...
        <div className="space-y-3">
          ...
          {soutesList.length === 0 ? (
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-8 text-center text-gray-400">
              ...
              <div className="text-sm mt-2">Cliquez sur "Ajouter" pour créer une soute</div>
            </div>
          ) : (
            soutesList.map(soute => (
              <SouteCard
                ...
                onSave={(updates) => {
                  updateSoute(activeModel.id, soute.id, updates)
                  setEditingSoute(null)
                }}
                onCancel={() => setEditingSoute(null)}
                onDelete={() => {
                  if (confirm(`Supprimer ${soute.nom} ?`)) {
                    deleteSoute(activeModel.id, soute.id)
                  }
                }}
              />
            ))
      ...
      <div className="bg-gray-900 border-t border-gray-800 p-4 grid grid-cols-2 gap-2">
        ...
        <button
          onClick={() => {
            if (confirm(`Supprimer ${activeModel.nom} ?`)) {
              deleteModel(activeModel.id)
            }
          }}
  ...
  if (isEditing) {
    return (
      <div className="bg-gray-800 border border-blue-500 rounded-lg p-4">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-blue-400">📝 ÉDITION MODÈLE</h3>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) => setFormData({...formData, drapeau: e.target.value})}
                className="w-16 bg-gray-900 border border-gray-600 rounded px-2 py-2 text-center text-xl"
                maxLength={2}
              />
              <input
                type="text"
                ...
                className="flex-1 bg-gray-900 border border-gray-600 rounded px-3 py-2 text-white font-semibold"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={formData.masseVide}
              onChange={(e) => setFormData({...formData, masseVide: parseInt(e.target.value) || 0})}
          ...
          <div>
            ...
            <input
              type="number"
              value={formData.cgVide}
              onChange={(e) => setFormData({...formData, cgVide: parseInt(e.target.value) || 0})}
              className="w-full bg-gray-900 border border-gray-600 rounded px-3 py-2 text-white font-semibold"
            />
          </div>
    ...
    <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
      ...
      <div className="grid grid-cols-3 gap-3 text-sm">
        <div className="text-center">
          <div className="text-gray-400">Masse vide</div>
          <div className="text-white font-semibold">{model.masseVide}g</div>
        </div>
        <div className="text-center">
          <div className="text-gray-400">CG vide</div>
          <div className="text-white font-semibold">{model.cgVide}mm</div>
        </div>
        <div className="text-center">
          <div className="text-gray-400">Surface</div>
          <div className="text-white font-semibold">{model.surface}dm²</div>
        </div>
      </div>
    </div>
  )
...
function SouteCard({ soute, modelId, cgVide, isEditing, onEdit, onSave, onCancel, onDelete }) {
  ...
  return (
    <div 
      className="border-2 bg-gray-800 rounded-lg p-4"
      style={{ borderColor: soute.couleur }}
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onDelete}
            className="bg-red-900 hover:bg-red-800 p-2 rounded"
          >
            <Trash2 size={16} />
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm mb-3">
        <div>
          <div className="text-gray-400">Distance BA</div>
          <div className="text-white font-semibold">{soute.distanceBA} mm</div>
        </div>
        <div>
          <div className="text-gray-400">Position CG</div>
          <div className="text-white font-semibold">
            {positionRelative > 0 ? '+' : ''}{positionRelative} mm
          </div>
        </div>
        <div className="col-span-2">
          <div className="text-gray-400">Capacité</div>
          <div className="text-white font-semibold">{soute.capacite} blocs par côté</div>
        </div>
      </div>
          ...
          {soute.materiaux?.map((mat, i) => (
            <div key={i} className="flex-1 bg-gray-900 rounded px-2 py-1 text-center">
              ...
              <div className="text-gray-500 text-xs">
                {mat.stock !== null ? `Stock: ${mat.stock}` : '∞'}
              </div>
            </div>
