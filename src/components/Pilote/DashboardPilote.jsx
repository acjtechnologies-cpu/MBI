...
export default function DashboardPilote() {
  ...
  const { config, setState } = useSouteStore()
  ...
  const solution = resoudreSoute(masseCibleGrammes, masseVideGrammes, 0, config) // offset déjà dans masseCible
  const stats = calculerStatsSoute(solution, config)
          ...
          return (
            <div key={pack} className="flex justify-center gap-0.5 items-center">
              <div className="flex gap-0.5 w-[48%] h-20 flex-row-reverse">
                {renderSlots(gaucheData, config, pack)}
              </div>
              <div className="flex gap-0.5 w-[48%] h-20">
                {renderSlots(droiteData, config, pack)}
              </div>
            </div>
          )
        })}
...
function renderSlots(materials, config, pack) {
  ...
  for (let i = 0; i < capacity; i++) {
    ...
    if (material === 'W') bgClass = 'bg-[#cd7f32]'
    if (material === 'L') bgClass = 'bg-[#c0c0c0]'
    if (material === 'B') bgClass = 'bg-[#ffd700]'
