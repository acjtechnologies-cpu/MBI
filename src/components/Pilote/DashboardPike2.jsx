...
// ── Données Pike Precision 2 (source : pike2-v3.html) ──────────────────────
const MV = 2332
...
// ── CSS inline (classes préfixées p2- pour éviter collisions) ──────────────
const CSS = `
.p2-app { height:100dvh; overflow:hidden; background:#05070a; color:#fff;
...
export default function DashboardPike2() {
  ...
  function selectParam(p) { setSelectedParam(p) }
  ...
  const hints = {
    vent: alt > 0
      ? `Table 0m → Baro ${alt}m → +1g → ${kgVal.toFixed(3)} kg`
      : `Config #${cfg.n} — ${cfg.m}g (Δ${dm > 0 ? '+' : ''}${dm}g)`,
    offset: `Pas 42g · total: ${offset >= 0 ? '+' : ''}${offset}g`,
    alt: (() => {
      const c = Math.round((m0kg - getMasseAlt(m0kg, 100)) * 1000)
      return `Pas 50m • ≈−${c}g/100m à ${vent.toFixed(1)} m/s`
    })(),
  ...
  function doChange(dir) {
    if (selectedParam === 'vent') {
      ...
      if ((offset < 0 && newOff > 0) || (offset > 0 && newOff < 0)) {
        useAppStore.getState().setOffset(0)
      } else {
        dir > 0 ? incrementOffset() : decrementOffset()
      }
  ...
  function renderSlots(slots, cap) {
    ...
    for (let i = 0; i < cap; i++) {
      ...
      let cls = 'p2-s'
      if (b) {
        if (b.cls === 't70') cls = 'p2-t'
        else if (b.cls === 'l126' || b.cls === 'l63') cls = 'p2-lb'
        else cls = 'p2-l'
      }
      items.push(<div key={i} className={`p2-slot ${cls}`} />)
    }
    return items
  }
  ...
  function renderMatrixSide(slots, maxN) {
    ...
    return full.map((slot, i) => (
      <div key={i} className={`p2-mc${slot ? ' ' + slot.cls : ''}`}>
        {slot && (
          <>
            <div className="p2-mc-w">{slot.lbl}</div>
            <div className="p2-mc-t">{slot.typ}</div>
          </>
        )}
      </div>
    ))
  ...
  const calcM  = selCfg
    ? MV + [...selCfg.avL, ...selCfg.avR, ...selCfg.arL, ...selCfg.arR].reduce((s, c) => s + c.w, 0)
    : 0
    ...
    <>
      ...
      <div className="p2-app" translate="no">
        ...
        <div className="p2-tabs">
          <button
            className={`p2-tab${tab === 'calc' ? ' on' : ''}`}
            onClick={() => setTab('calc')}
          >⚖ CALCULATEUR</button>
          <button
            className={`p2-tab${tab === 'matrix' ? ' on' : ''}`}
        ...
        {tab === 'calc' && (
          <div className="p2-calc">
            ...
            {/* Header vent */}
            <div
              className={`p2-vent${selectedParam === 'vent' ? ' active' : ''}`}
            ...
            <div className="p2-data">
              ...
              <div style={{ textAlign: 'center' }}>
                <div className={`p2-cg${Math.abs(cgD) < 0.6 ? ' neutre' : cgD < 0 ? ' avant' : ' arriere'}`}>
                  ...
                  <div className="p2-data-lbl">
                    {Math.abs(cgD) < 0.6 ? '✓ CG' : cgD < 0 ? cgD.toFixed(1) + 'mm' : '+' + cgD.toFixed(1) + 'mm'}
                  </div>
                </div>
              </div>
            </div>
            ...
            {alt > 0 && (
              <div className="p2-alt">
                <div style={{ textAlign: 'center', flex: 1 }}>
                  <span className="p2-ab-lbl">ALTITUDE</span>
                  <span className="p2-ab-val">{alt} m</span>
                </div>
                <div style={{ textAlign: 'center', flex: 1 }}>
                  <span className="p2-ab-lbl">DÉDUIT</span>
                  <span className="p2-ab-val">−{Math.round((m0kg - mAltkg) * 1000)} g</span>
                </div>
                <div style={{ textAlign: 'center', flex: 1 }}>
                  <span className="p2-ab-lbl">FINALE</span>
                  <span className="p2-ab-val">{kgVal.toFixed(3)} kg</span>
                </div>
              </div>
            )}
              ...
              <div className="p2-ctrl-grid">
                <div className="p2-ctrl-left">
                  <div className="p2-ctrl-top2">
                    <div
                      className={`p2-mode-btn${selectedParam === 'kg' ? ' active' : ''}`}
                    ...
                    <div
                      className={`p2-mode-btn${selectedParam === 'alt' ? ' active-alt' : ''}`}
                  ...
                  <div
                    className={`p2-mode-btn${selectedParam === 'offset' ? ' active' : ''}`}
                    onClick={() => selectParam('offset')}
                  >
                    <span className="p2-mode-val">{offset >= 0 ? '+' : ''}{offset}g</span>
                    <span className="p2-mode-lbl">OFFSET</span>
                  </div>
                </div>
        ...
        {tab === 'matrix' && (
          <div className="p2-matrix">
            <div className="p2-m-hdr">
              <div>
                ...
                <div style={{ fontSize: 9, color: '#8b949e', marginTop: 1 }}>
                  {selCfg
                    ? `Config #${selCfg.n} · ${selCfg.avL.length + selCfg.avR.length}S · ${selCfg.arL.length + selCfg.arR.length}B`
                    : 'Sélectionner une configuration'}
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 14, fontWeight: 900, color: '#3fb950' }}>
                  {selCfg ? selCfg.m + ' g ref' : '—'}
                </div>
                <div style={{ fontSize: 9, color: '#8b949e' }}>
                  {selCfg ? `calc ${calcM}g (${calcM - selCfg.m > 0 ? '+' : ''}${calcM - selCfg.m}g)` : '—'}
                </div>
              </div>
            </div>
            ...
            {!selCfg && (
              <div className="p2-m-ph">← Choisir une configuration</div>
            )}
              ...
              <>
                <div className="p2-m-soutes">
                  <div className="p2-m-sw av">
                    <div className="p2-m-lbl" style={{ color: 'rgba(200,160,48,.9)' }}>
                      ⬆ AV — Small (42g L · 21g ½L · 70g T)
                    </div>
                  ...
                  <div className="p2-m-sw ar">
                    <div className="p2-m-lbl" style={{ color: 'rgba(112,128,144,.9)' }}>
                      ⬇ AR — Big (126g L · 63g ½L)
                    </div>
                    <div className="p2-m-row">
                ...
                <div className="p2-m-info">
                  <div>
                    <div style={{
                      ...
                      color: Math.abs(calcM - selCfg.m) <= 30 ? '#3fb950' : '#f0a500'
                    }}>{calcM} g</div>
                    ...
                    <div style={{
                      ...
                      {calcM === selCfg.m
                        ? '= ref'
                        : (calcM - selCfg.m > 0 ? '+' : '') + (calcM - selCfg.m) + 'g vs ref'}
                    </div>
                  </div>
                  <div style={{ fontSize: 9, color: '#8b949e', flex: 2, lineHeight: 1.4 }}>
                    AV <span style={{ color: '#fff', fontWeight: 700 }}>
                      {[...selCfg.avL, ...selCfg.avR].reduce((s, c) => s + c.w, 0)}g
                    </span> ({selCfg.avL.length + selCfg.avR.length} blocs)<br />
                    AR <span style={{ color: '#fff', fontWeight: 700 }}>
                      {[...selCfg.arL, ...selCfg.arR].reduce((s, c) => s + c.w, 0)}g
                    </span> ({selCfg.arL.length + selCfg.arR.length} blocs)
                  </div>
                </div>
              </>
            )}
            ...
            <div className="p2-m-leg">
              {[
                ...
                <div key={lbl} className="p2-m-li">
                  <div className="p2-m-ls" style={{ background: bg }} />
                  {lbl}
                </div>
              ))}
            </div>
